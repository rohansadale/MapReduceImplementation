		...
		try{
			...
			return;
                ...
                if(activeNodes!=null) //checking whether current node is able to join network or not
                {
                        ...
                        String hashKeyMaster = configParam.get(SERVER_IP_KEY) + configParam.get(SERVER_PORT_KEY);
                        Node coordinatorNode = new Node(configParam.get(SERVER_IP_KEY),Integer.parseInt(configParam.get(SERVER_PORT_KEY)),
                                                                                                        Util.getInstance().hash(hashKeyMaster));
